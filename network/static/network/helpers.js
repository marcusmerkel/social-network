function load_posts(category, page) {
  // category i.e. "all", "following"
  fetch(`/posts/${category}/${page}`).then(response => response.json()).then(posts => {
    console.log(posts);
    ...
    if (category === "all") {
      ReactDOM.render( /*#__PURE__*/React.createElement(NewPostForm, null), document.querySelector('#new_post'));
...
function load_user(username) {
  ...
  fetch(`/profile/${username}/data`).then(response => response.json()).then(data => {
    ReactDOM.render( /*#__PURE__*/React.createElement(ProfileHeader, {
  ...
  fetch(`/posts/${username}/count`).then(response => response.json()).then(result => {
    const post_count = result.post_count;
    const page_count = result.page_count;

    if (result.post_count > 10) {
      ReactDOM.render( /*#__PURE__*/React.createElement(PageNav, {
        category: username,
        page_count: result.page_count
      }), document.querySelector('#page-nav-container'));
...
function add_post(post) {
  ...
  ReactDOM.render( /*#__PURE__*/React.createElement(Post, {
    key: post.id,
    id: post.id,
    text: post.text,
    author: post.author,
